...
## 10. Trainium 的推理是 bit-level batch-invariant 的（2026-09-11）

GPU 侧确定性推理文献（TML batch-invariant kernels、SGLang deterministic mode）要手写 kernel 才能达到的性质，Trainium 上是**免费的**。

同一批 32 条前缀，三种提交方式，比较 logprob（`tools/probe_batch_invariance*.py`）：

| 路径 | 对照 | token 序列相同 | logprob 逐位相同 | max\|Δ\| |
|---|---|---|---|---|
| prefill (CTE) | 批 32 重跑（控制组） | — | 8224/8224 | 0 |
| prefill (CTE) | **批 32 vs 批 1** | — | **8224/8224** | **0** |
| prefill (CTE) | **批 32 顺序反转（槽位不同）** | — | **8224/8224** | **0** |
| decode (TKG), 贪心 | **批 32 vs 批 1** | **32/32** | **2048/2048** | **0** |
| decode (TKG), 贪心 | **批 32 顺序反转** | **32/32** | **2048/2048** | **0** |

机制：静态形状。NEFF 在编译期就固定了 batch 维度（= `max_num_seqs`），活跃序列数变化只改变 padding 行的内容，归约树不变。

### 10.1 一条真实的架构权衡

**同一个静态形状设计，在验证侧收费、在确定性侧付钱：**
- 收费（§7.3）：每次验证 prefill 必须付整桶成本，无法按前缀长度缩短——这是验证-回滚开销的地板。
- 付钱（本节）：batch/槽位不变性免费到手，而 GPU 需要为此重写 kernel 并付 34–135% 的开销。
...
| 来源 | 量级 |
|---|---|
| prefill 图 vs decode 图（同编译器，§7.1 的 B） | 0.0167 |
| 编译器 A vs 编译器 B（同为 prefill，§7.1 的 C） | 0.0160 |
| **批组成 / 槽位** | **严格为 0** |

在 GPU 上，train/infer 失配里有一部分是批依赖，可以用 batch-invariant kernel 修掉；**在 Trainium 上这一部分恰好为零**。所以那条路线在这里既非必要（已经有了）也不充分（修不了两个编译器的差异）。

### 10.3 附带发现：vLLM 的带 seed 采样不是 batch-invariant

同样的 32 条前缀、同样的 per-request seed，批 32 vs 批 1：**token 序列 0/32 相同，首次分叉位置的中位数 = 0**。而第 0 步的 logits 已证明逐位相同（上表第 2 行）。即：分布相同、seed 相同，采出的 token 却不同——vLLM（本版本 V1）的带 seed 采样其随机数抽取依赖批内位置。

这是软件性质，与 Trainium 无关，但对 RL 实践是个坑：**无法通过单独重跑一个 prompt 来复现某次 rollout**。§3.2 的 512/512 复现能通过，是因为重跑时批组成完全相同。

（记录一次差点发生的误判：最初用带 seed 的采样测 decode，得到 0/32，看起来像"decode 不是 batch-invariant"。破绽是**顺序反转组也是 0/32**——反转不改变任何前向输入，数值上不可能分叉。改用贪心解码隔离 RNG 后结论反转。）

---

## 11. 长度标度的直接验证（2026-09-12）—— 修正 §8.2

§8.2 的 ESS-vs-T 表基于 σ ∝ √T 的外推（只有 T=511、868 两点）。直接跑了 T=1900（桶 2048，512 条，采样 68 min，重算 173 s），并改用**单次 run 内部**测量以消除跨 run 混杂。

### 11.1 σ(T) ∝ T^0.445，且误差跨位置独立
...
**重建列（假设 Δ_t 跨位置独立，σ² = Σ_t Var(Δ_t)）与实测列吻合** → 独立性在 15× 的长度范围上成立，与 §2.2 的 lag-1 自相关 ≈ 0 一致。

### 11.2 偏离 √T 的原因就是失配定律

指数 0.445 < 0.5，因为 std(Δ_t) 随位置下降；而定律说它应当正比于 E[1−p_t]：

| 位置区间 | std(Δ_t) | E[1−p_t] | 比值 |
|---|---|---|---|
| [0,16) | 0.0393 | 0.3062 | 0.128 |
| [16,64) | 0.0320 | 0.1706 | 0.188 |
| [64,256) | 0.0312 | 0.1614 | 0.193 |
| [256,512) | 0.0309 | 0.1752 | 0.177 |
| [512,1024) | 0.0278 | 0.1588 | 0.175 |
| [1024,1900) | 0.0204 | 0.1046 | 0.195 |

比值在位置 16 之后恒定在 0.175–0.195。**§9 的定律不仅预测失配的大小，还预测它的长度标度**——σ 的次平方根增长完全是"后面的 token 更自信"的推论，不是误差相关性。

### 11.3 修正后的外推表（替代 §8.2）

| T | σ | **序列级 IS 的 ESS/N** | §8.2 的旧 √T 估计 | 验证-回滚 wall-clock |
|---|---|---|---|---|
| 512 | 0.66 | 0.650 | 0.60 | 19%（实测） |
| 1024 | 0.89 | 0.450 | 0.36 | 18%（实测） |
| 1883 | 1.17 | **0.253**（实测） | — | — |
| 4096 | 1.66 | **0.064** | 0.018 | — |
| 8192 | 2.25 | **0.006** | 0.000 | — |

旧表偏悲观约 2–3×，但结论不变：**序列级重加权在 4k 以上的 rollout 上不可用（ESS < 7%），而验证-回滚的 wall-clock 对 T 持平。**

### 11.4 附带：λ 随长度下降

T=1883 上 λ = 0.00444，低于 T=511 的 0.00579（−23%），同样是置信度效应。含义：**验证-回滚的每 token 拒绝率在长序列上更低，其成本随 T 次线性增长**——这与 §3.3 实测的 wall-clock 从 T=512 到 T=1024 基本持平（19% → 18%）一致。
